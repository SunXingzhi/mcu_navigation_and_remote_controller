...
extern uint8_t pid_status;				   // PID状态
// 导航信息全局变量
NavigationInfo navigation_info = {
	...
	.left_motor_duty_cycle = 0,	  // 左侧电机的占空比
	.right_motor_duty_cycle = 0,	 // 右侧电机的占空比
...
// 接收转向运动命令
void Navigation_ReceiveTurnInfo(char* received_data_buffer)
{
	char* token;
	char buffer[50];
	int32_t motor_duty_cycle;
	...
	// 设置状态为执行转向
	navigation_info.state = NAVIGATION_EXECUTING_TURN;
}
...
void Navigation_Execute()
{
	...
	motor_information.motor_duty_cycle[MOTOR_LEFT_FORWARD] = 
		...
		navigation_info.left_motor_duty_cycle : -navigation_info.left_motor_duty_cycle;
	...
	motor_information.motor_duty_cycle[MOTOR_RIGHT_FORWARD] = 
		(navigation_info.right_motor_duty_cycle >= 0) ? 
		navigation_info.right_motor_duty_cycle : -navigation_info.right_motor_duty_cycle;
